...
void print_usage(const char *prog) {
  std::cout
      << "Usage: " << prog << " [options]\n\n"
      << "Options:\n"
      << "  --mode [independent|shared|all]  Stack mode to test (default all)\n"
...
int run_server_process(const BenchConfig &cfg, const std::string &mode,
  ...
  try {
    ...
    builder.get(cfg.path, [](const zhttp::HttpRequest::ptr &, zhttp::HttpResponse &resp) {
...
ModeResult run_mode(const BenchConfig &base_cfg, const std::string &mode,
  ...
  if (::pipe(ready_pipe) != 0) {
    ...
    mode_result.wrk.output = std::string("ready pipe failed: ") +
                             std::strerror(errno);
    return mode_result;
  }
  ...
  if (::pipe(log_pipe) != 0) {
    ...
    ::close(ready_pipe[1]);
    mode_result.wrk_exit_code = 1;
    mode_result.wrk.output = std::string("log pipe failed: ") +
                             std::strerror(errno);
    return mode_result;
  }
  ...
  if (server_pid < 0) {
    ...
    ::close(log_pipe[0]);
    mode_result.wrk_exit_code = 1;
    mode_result.wrk.output = std::string("server fork failed: ") +
                             std::strerror(errno);
    return mode_result;
  }
...
void print_mode_result(const ModeResult &result) {
  std::cout << "\n===== " << result.mode << " stack =====\n";
  std::cout << "port=" << result.port << ", server_started="
            << (result.server_started ? "yes" : "no")
            << ", server_exit="
