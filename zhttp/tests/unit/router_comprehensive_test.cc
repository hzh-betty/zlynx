...
TEST_F(RouterComprehensiveTest, Static_DeepPath) {
    ...
    HttpResponse resp;
    EXPECT_TRUE(
        router_.route(make_request(HttpMethod::GET, "/a/b/c/d/e"), resp));
}
...
TEST_F(RouterComprehensiveTest, Static_DeleteMethod) {
    ...
    HttpResponse resp;
    EXPECT_TRUE(
        router_.route(make_request(HttpMethod::DELETE, "/remove"), resp));
}
...
TEST_F(RouterComprehensiveTest, Static_SpecialCharsInPath) {
    ...
    HttpResponse resp;
    EXPECT_TRUE(
        router_.route(make_request(HttpMethod::GET, "/api/v1.0"), resp));
}

TEST_F(RouterComprehensiveTest, Static_HyphenInPath) {
    router_.get("/my-api-path", [](const HttpRequest::ptr &,
                                   HttpResponse &resp) { resp.text("ok"); });
    HttpResponse resp;
    EXPECT_TRUE(
...
TEST_F(RouterComprehensiveTest, Static_UnderscoreInPath) {
    router_.get("/my_api_path", [](const HttpRequest::ptr &,
                                   HttpResponse &resp) { resp.text("ok"); });
    HttpResponse resp;
    EXPECT_TRUE(
...
TEST_F(RouterComprehensiveTest, Static_AllHttpMethods) {
    ...
    EXPECT_TRUE(
        router_.route(make_request(HttpMethod::OPTIONS, "/options"), r2));
...
TEST_F(RouterComprehensiveTest, Dynamic_StaticPriorityOverParam) {
    router_.get("/users/admin", [](const HttpRequest::ptr &,
                                   HttpResponse &resp) { resp.text("admin"); });
...
TEST_F(RouterComprehensiveTest, Extra_NestedParams) {
    router_.get("/org/:org/team/:team/user/:user",
                [](const HttpRequest::ptr &req, HttpResponse &resp) {
                    resp.text(req->path_param("org") + "/" +
                              req->path_param("team") + "/" +
                              req->path_param("user"));
                });
    HttpResponse resp;
...
TEST_F(RouterComprehensiveTest, Extra_StaticAndDynamicMix) {
    router_.get("/api/status", [](const HttpRequest::ptr &,
                                  HttpResponse &resp) { resp.text("status"); });
    router_.get("/api/:resource",
                [](const HttpRequest::ptr &req, HttpResponse &resp) {
                    resp.text("resource:" + req->path_param("resource"));
                });
    router_.get("/api/config", [](const HttpRequest::ptr &,
                                  HttpResponse &resp) { resp.text("config"); });
...
TEST_F(RouterComprehensiveTest, Extra_CatchAllPrecedence) {
    router_.get("/files/:name", [](const HttpRequest::ptr &,
                                   HttpResponse &resp) { resp.text("param"); });
    router_.get("/files/*path",
                [](const HttpRequest::ptr &, HttpResponse &resp) {
                    resp.text("catchall");
                });
...
TEST_F(RouterComprehensiveTest, Extra_LargeNumberRoutes) {
    ...
    HttpResponse resp;
    EXPECT_TRUE(
        router_.route(make_request(HttpMethod::GET, "/large499"), resp));
