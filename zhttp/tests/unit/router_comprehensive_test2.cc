...
TEST_F(RouterComprehensiveTest2, Regex_MultipleCaptures) {
    router_.add_regex_route(
        HttpMethod::GET, "/date/(\\d{4})-(\\d{2})-(\\d{2})",
        {"year", "month", "day"},
        [](const HttpRequest::ptr &req, HttpResponse &resp) {
            resp.text(req->path_param("year") + "/" + req->path_param("month") +
                      "/" + req->path_param("day"));
        });
    HttpResponse resp;
...
TEST_F(RouterComprehensiveTest2, Regex_AlphaNumeric) {
    router_.add_regex_route(
        HttpMethod::GET, "/items/([a-zA-Z0-9]+)", {"code"},
        [](const HttpRequest::ptr &req, HttpResponse &resp) {
...
TEST_F(RouterComprehensiveTest2, Regex_OptionalPart) {
    router_.add_regex_route(
        HttpMethod::GET, "/api/v(\\d+)(?:/.*)?", {"version"},
        [](const HttpRequest::ptr &req, HttpResponse &resp) {
...
TEST_F(RouterComprehensiveTest2, Regex_EmailLike) {
    router_.add_regex_route(
        ...
        [](const HttpRequest::ptr &req, HttpResponse &resp) {
            resp.text(req->path_param("user") + "@" +
                      req->path_param("domain"));
        });
    HttpResponse resp;
...
TEST_F(RouterComprehensiveTest2, Regex_PrefixBucket_SamePrefix) {
    router_.add_regex_route(HttpMethod::GET, "/api/v1/users/(\\d+)", {"id"},
                            [](const HttpRequest::ptr &, HttpResponse &resp) {
                                resp.text("users");
                            });
    router_.add_regex_route(HttpMethod::GET, "/api/v1/users/(\\d+)/posts",
                            {"id"},
                            [](const HttpRequest::ptr &, HttpResponse &resp) {
                                resp.text("posts");
                            });
    ...
    router_.route(make_request(HttpMethod::GET, "/api/v1/users/1/posts"),
                  resp2);
...
TEST_F(RouterComprehensiveTest2, Regex_Hyphen) {
    router_.add_regex_route(
        HttpMethod::GET, "/slug/([a-z0-9-]+)", {"slug"},
        [](const HttpRequest::ptr &req, HttpResponse &resp) {
            resp.text(req->path_param("slug"));
        });
    HttpResponse resp;
    router_.route(make_request(HttpMethod::GET, "/slug/my-cool-post-123"),
                  resp);
...
TEST_F(RouterComprehensiveTest2, Middleware_ExecutionOrder) {
    ...
    router_.get("/test",
                [&order](const HttpRequest::ptr &, HttpResponse &resp) {
                    order += "handler;";
                    resp.text("ok");
                });
...
TEST_F(RouterComprehensiveTest2, Edge_DotInPath) {
    ...
    HttpResponse resp;
    EXPECT_TRUE(
        router_.route(make_request(HttpMethod::GET, "/file.json"), resp));
}
...
TEST_F(RouterComprehensiveTest2, Edge_EncodedSlash) {
    ...
    HttpResponse resp;
    router_.route(make_request(HttpMethod::GET, "/path%2Fwith%2Fslashes"),
                  resp);
}
