...
protected:
  ...
  void advance_ms(int64_t ms) {
    t0_ += std::chrono::milliseconds(ms);
  }
};
...
TEST_F(RateLimiterTest, FixedWindow_BlocksAfterCapacity_ThenResets) {
  auto limiter = RateLimiter::newRateLimiter(RateLimiter::Type::FIXED_WINDOW, 2,
...
TEST_F(RateLimiterTest, SlidingWindow_SmoothsBoundary) {
  auto limiter = RateLimiter::newRateLimiter(RateLimiter::Type::SLIDING_WINDOW, 2,
                                             RateLimiter::TimeUnit::SECOND,
                                             now());
...
TEST_F(RateLimiterTest, TokenBucket_AllowsBurstAndRefill) {
  ...
  auto limiter = RateLimiter::newRateLimiter(RateLimiter::Type::TOKEN_BUCKET, 2,
                                             RateLimiter::TimeUnit::SECOND,
                                             now());
...
TEST_F(RateLimiterTest, Middleware_UsesKeyFuncIsolation) {
  auto limiter = RateLimiter::newRateLimiter(RateLimiter::Type::FIXED_WINDOW, 1,
                                             RateLimiter::TimeUnit::SECOND,
                                             now());
