...
/**
 * @brief Session 中间件：从 Cookie 读取/写回 Session ID，并管理会话数据
 *
 * before():
 * - 尝试从请求 Cookie 读取 session id 并加载。
 * - 若加载失败且 create_if_missing=true，则创建新会话并注入到 request。
 *
 * after():
 * - 仅当会话为新建或被修改时，才 save() 并写回 Set-Cookie。
