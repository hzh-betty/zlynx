...
class HttpResponse {
  public:
    ...
    /**
     * @brief 获取所有 Set-Cookie 值（每个元素对应一个 Set-Cookie 头部的 value
     * 部分）
     */
     ...
     */
    HttpResponse &
    upgrade_to_websocket(WebSocketCallbacks callbacks,
     ...
     * @details chunked 路径常用 include_body=false 仅发响应头，body
     * 走后续分块发送。
     ...
     * @brief Cookie 附加选项
     * @details
     * 这些选项会被拼接到 Set-Cookie 响应头中，用于控制 Cookie
     * 的作用域和安全属性。
     */
