...
public:
   ...
   */
  static std::string map_to_relative_path(const std::string &path,
                                          const std::string &normalized_prefix);
   ...
   * 4) 遇到 .. 直接失败。
   */
  static bool sanitize_relative_path(const std::string &raw, std::string &out);
...
public:
   ...
   * @return true 表示成功
   */
  static bool write_file_binary(const std::string &path, const std::string &content);
   ...
   * @return true 表示成功
   */
  static bool get_last_modified(const std::string &path, std::string &last_modified);
