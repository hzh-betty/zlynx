...
private:
  ...
  bool is_authenticated_by_bearer_token(const HttpRequest::ptr &request) const;
...
public:
  ...
  struct Options {
    ...
    std::string session_roles_key; // 默认角色来源：Session 中该键保存的角色字符串
    bool require_all;              // true=必须包含全部 required roles；false=命中任一即可
...
private:
   ...
   */
  std::vector<std::string> resolve_request_roles(
