...
void init_logger(zlog::LogLevel::value level) {
    ...
    std::atomic_store_explicit(&zhttp_logger, logger,
                               std::memory_order_release);
}

zlog::Logger::ptr get_logger_ptr() {
    zlog::Logger::ptr logger =
        std::atomic_load_explicit(&zhttp_logger, std::memory_order_acquire);
    if (logger) {
        return logger;
    }
    ...
    return std::atomic_load_explicit(&zhttp_logger, std::memory_order_acquire);
}
