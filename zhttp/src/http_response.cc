...
void append_header_line(std::string *out, const std::string &key,
...
HttpResponse &
HttpResponse::upgrade_to_websocket(WebSocketCallbacks callbacks,
...
void HttpResponse::serialize_to(std::string *out, bool include_body) const {
    ...
    // 如果业务层没手动设置 Content-Length，这里自动补齐，避免客户端无法判断
    // Body 边界。
