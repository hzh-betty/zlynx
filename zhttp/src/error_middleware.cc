...
void ErrorMiddleware::after(const HttpRequest::ptr &request,
  ...
  if (options_.only_format_when_body_empty && !response.body_content().empty()) {
    return;
  }
...
std::string ErrorMiddleware::build_error_json(const HttpRequest::ptr &request,
