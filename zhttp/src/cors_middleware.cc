...
bool CorsMiddleware::is_preflight_request(const HttpRequest::ptr &request) const {
  // 预检请求的最小判定：
  // - 方法是 OPTIONS；
  // - 携带 Origin；
...
std::string CorsMiddleware::resolve_allow_origin(const std::string &origin) const {
  // - 请求里带 Origin 时优先回显；
  // - 请求里无 Origin 时回退为 "*"。
  if (origin.empty()) {
    return "*";
...
bool CorsMiddleware::before(const HttpRequest::ptr &request,
  ...
  const std::string origin = request->header(kHeaderOrigin);
  if (!is_origin_allowed(origin) && options_.forbid_disallowed_origin_on_preflight) {
