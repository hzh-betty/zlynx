...
bool if_none_match_matches(const std::string &if_none_match,
  ...
  // 4) 仅做 token 层匹配，不解析复杂引号转义（当前 ETag 生成策略无需该复杂度）。
  //
  // 说明：这里用于 If-None-Match，按 RFC 可使用弱比较；If-Match 则通常需要强比较。
...
void apply_validator_headers(HttpResponse &response,
                             const std::string &etag,
...
bool try_handle_conditional_not_modified(
  ...
  if (options.enable_last_modified && request->header("If-None-Match").empty()) {
...
bool StaticFileMiddleware::before(const HttpRequest::ptr &request,
  ...
  std::string disk_path = PathOperator::join_path(options_.document_root, relative);
  ...
  const std::string content_type = FileOperator::detect_content_type(disk_path);
  ...
  if (options_.enable_memory_cache && options_.memory_cache_time > 0) {
    ...
    for (const auto &enc : encoding_candidates) {
      ...
      if (TimerHelper::steady_now() <= it->second.expires_at) {
        ...
        write_payload_by_range(request,
                               response,
                               parsed_range,
  ...
  apply_entity_headers(
      response, options_, content_type, content_encoding, etag, last_modified);

  const ParsedRange parsed_range =
      parse_range_request(request, content.size(), last_modified);

  write_payload_by_range(request, response, parsed_range, content.size(), content);
  response.set_keep_alive(request->is_keep_alive());
