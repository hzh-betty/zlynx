...
class Channel {
 public:
  ...
  bool read(T& out, uint32_t timeout_ms) const {
    while (true) {
      ...
      if (!not_empty_.wait(timeout_ms)) {
        done_.store(false, std::memory_order_release);
        ZCOROUTINE_LOG_DEBUG("channel read timeout, timeout_ms={}", timeout_ms);
        return false;
      }
    }
  }
   ...
   * @return true 表示写入成功。
   */
  bool try_write(const T& value) const {
   ...
   * @return true 表示写入成功。
   */
  bool try_write(T&& value) const {
    return write(std::move(value), 0);
  }
  ...
  bool write_impl(T&& value, uint32_t timeout_ms) const {
    while (true) {
      ...
      if (!not_full_.wait(timeout_ms)) {
        done_.store(false, std::memory_order_release);
        ZCOROUTINE_LOG_DEBUG("channel write timeout, timeout_ms={}", timeout_ms);
        return false;
      }
    }
  }
