...
class Logger {
 public:
  ...
  void debug(const char*, std::size_t, const char*, Args&&...) {}

  template <typename... Args>
  void info(const char*, std::size_t, const char*, Args&&...) {}

  template <typename... Args>
  void warning(const char*, std::size_t, const char*, Args&&...) {}

  template <typename... Args>
  void error(const char*, std::size_t, const char*, Args&&...) {}

  template <typename... Args>
  void fatal(const char*, std::size_t, const char*, Args&&...) {}
};
...
inline void init_logger(const LoggerInitOptions&) {}
inline void init_logger(zlog::LogLevel::value = zlog::LogLevel::value::INFO) {}

inline zlog::Logger::ptr& cached_logger() {
  static zlog::Logger::ptr logger = std::make_shared<zlog::Logger>();
  return logger;
}

inline zlog::Logger* get_logger() {
  return cached_logger().get();
}
