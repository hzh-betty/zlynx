...
void require_eq(const T &actual, const U &expected,
...
int scaled_workload(int base_value, int scale_pct) {
    const int scaled =
        static_cast<int>((static_cast<int64_t>(base_value) * scale_pct) / 100);
    return scaled > 0 ? scaled : 1;
}

WorkloadConfig load_workload_config() {
    const int scale_pct =
        read_env_int("ZCOROUTINE_PERF_SCALE_PCT", 100, 1, 1000);

    WorkloadConfig config;
    config.scheduler_count = read_env_int("ZCOROUTINE_PERF_SCHED_COUNT",
                                          kDefaultSchedulerCount, 1, 256);
    config.producer_threads = read_env_int("ZCOROUTINE_PERF_PRODUCER_THREADS",
                                           kDefaultProducerThreads, 1, 256);
    config.scheduler_tasks =
        scaled_workload(read_env_int("ZCOROUTINE_PERF_SCHED_TASKS",
                                     kDefaultSchedulerTasks, 1, INT_MAX),
                        scale_pct);
    config.yield_interval = read_env_int("ZCOROUTINE_PERF_YIELD_INTERVAL",
                                         kDefaultYieldInterval, 1, INT_MAX);
    config.stack_size =
        read_env_size_t("ZCOROUTINE_PERF_STACK_SIZE", kDefaultStackSize,
                        16 * 1024, 8 * 1024 * 1024);
    config.shared_stack_num = read_env_size_t(
        "ZCOROUTINE_PERF_SHARED_STACK_NUM", kDefaultSharedStackNum, 1, 1024);
    config.channel_messages =
        scaled_workload(read_env_int("ZCOROUTINE_PERF_CHANNEL_MESSAGES",
                                     kDefaultChannelMessages, 1, INT_MAX),
                        scale_pct);
    config.timer_tasks =
        scaled_workload(read_env_int("ZCOROUTINE_PERF_TIMER_TASKS",
                                     kDefaultTimerTasks, 1, INT_MAX),
                        scale_pct);
    config.hook_rounds =
        scaled_workload(read_env_int("ZCOROUTINE_PERF_HOOK_ROUNDS",
...
ScenarioResult run_scheduler_throughput(StackModel model,
    ...
    for (int t = 0; t < config.producer_threads; ++t) {
        producers.emplace_back([t, &done, &executed, &config]() {
            const int begin =
                (config.scheduler_tasks * t) / config.producer_threads;
            const int end =
                (config.scheduler_tasks * (t + 1)) / config.producer_threads;
    ...
    require_eq(executed_count, config.scheduler_tasks,
               "scheduler_submit executed mismatch");
    ...
    return ScenarioResult{"scheduler_submit",
                          model,
                          config.producer_threads,
                          config.scheduler_tasks,
                          seconds,
                          static_cast<double>(config.scheduler_tasks) /
                              seconds};
}
...
ScenarioResult run_channel_throughput(StackModel model,
    ...
    require_true(producer_ok.load(std::memory_order_acquire),
                 "channel producer failed");
    require_true(consumer_ok.load(std::memory_order_acquire),
                 "channel consumer failed");
    require_eq(produced.load(std::memory_order_relaxed),
               config.channel_messages, "channel produced count mismatch");
    require_eq(consumed.load(std::memory_order_relaxed),
               config.channel_messages, "channel consumed count mismatch");
    ...
    return ScenarioResult{"channel_spsc",
                          model,
                          kProducerCoroutines + kConsumerCoroutines,
                          config.channel_messages,
                          seconds,
                          static_cast<double>(config.channel_messages) /
                              seconds};
}
...
ScenarioResult run_timer_throughput(StackModel model,
    ...
    return ScenarioResult{"timer_sleep",
                          model,
...
ScenarioResult run_hook_io_throughput(StackModel model,
    ...
    if (::fcntl(writer.get(), F_SETFL,
                ::fcntl(writer.get(), F_GETFL, 0) | O_NONBLOCK) != 0 ||
        ::fcntl(reader.get(), F_SETFL,
                ::fcntl(reader.get(), F_GETFL, 0) | O_NONBLOCK) != 0) {
    ...
    require_true(writer_ok.load(std::memory_order_acquire),
                 "hook writer failed");
    require_true(reader_ok.load(std::memory_order_acquire),
                 "hook reader failed");
    ...
    return ScenarioResult{"hook_socketpair",
                          model,
                          2,
                          config.hook_rounds,
                          seconds,
...
int run_benchmark() {
    ...
    results.push_back(
        run_scheduler_throughput(StackModel::kIndependent, config));
    ...
    for (size_t i = 0; i < results.size(); ++i) {
        require_positive(results[i].throughput_ops_per_second,
                         std::string("non-positive throughput for scenario ") +
                             results[i].scenario);
