...
TEST_F(RuntimeStressTest, EightThreadsHundredThousandTasks) {
  ...
  const long long expected_sum =
      (static_cast<long long>(kTotalTasks - 1) * static_cast<long long>(kTotalTasks)) / 2;
...
TEST_F(RuntimeStressTest, RandomizedConcurrentSubmitPatternDeterministicSeed) {
  ...
  EXPECT_EQ(counter.load(std::memory_order_relaxed), kThreadCount * kRoundsPerThread);
}
  ...
  for (int round = 0; round < kRounds; ++round) {
    ...
    for (int t = 0; t < kThreadCount; ++t) {
      producers.emplace_back([&done, &total_counter, round, t]() {
        std::mt19937 rng(20260329u + static_cast<uint32_t>(round * 97 + t));
    ...
    EXPECT_EQ(total_counter.load(std::memory_order_relaxed), (round + 1) * kPerRoundTotal);
  }

  EXPECT_EQ(total_counter.load(std::memory_order_relaxed), kRounds * kPerRoundTotal);
}
