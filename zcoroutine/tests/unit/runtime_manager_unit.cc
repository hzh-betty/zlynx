...
TEST_F(RuntimeManagerUnitTest, PickProcessorIndexReturnsZeroWithSingleScheduler) {
  Runtime& runtime = Runtime::instance();
  runtime.init(1);
...
TEST_F(RuntimeManagerUnitTest, IndependentStackConfigResetsToDefaultsAfterShutdown) {
  Runtime& runtime = Runtime::instance();
...
TEST_F(RuntimeManagerUnitTest, IndependentStackBlockedCoroutinesHaveDistinctStackAddresses) {
  co_stack_model(StackModel::kIndependent);
  co_stack_size(64 * 1024);
  init(1);
  ...
  for (int i = 0; i < kFiberCount; ++i) {
    stack_addrs[static_cast<size_t>(i)].store(0u, std::memory_order_relaxed);
  }
  ...
  for (int i = 0; i < kFiberCount; ++i) {
    go([&ready, &done, &gate, &stack_addrs, i]() {
      ...
      stack_addrs[static_cast<size_t>(i)].store(
          reinterpret_cast<uintptr_t>(&local_marker), std::memory_order_release);
      ready.done();
      (void)gate.wait();
      done.done();
    });
...
TEST_F(RuntimeManagerUnitTest, RandomizedSubmitAndSchedulerDispatchDeterministicSeed) {
  ...
  for (int t = 0; t < kThreadCount; ++t) {
    producers.emplace_back([&runtime, &done, &counter, &scheduler_hits, t]() {
      ...
      for (int i = 0; i < kTasksPerThread; ++i) {
        ...
        const auto task = [&done, &counter, &scheduler_hits]() {
          ...
          if (sid >= 0 && static_cast<size_t>(sid) < scheduler_hits.size()) {
            scheduler_hits[static_cast<size_t>(sid)].fetch_add(1, std::memory_order_relaxed);
          }
          counter.fetch_add(1, std::memory_order_relaxed);
          done.done();
        };
