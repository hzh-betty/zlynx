...
bool strict_contract_failure(const char* func_name, const char* message, int err) {
  errno = err;
  ZCOROUTINE_LOG_FATAL("{} contract violation: {}", func_name, message);
#ifndef NDEBUG
  assert(false && "zcoroutine co_* contract violation");
#endif
  return false;
}
...
bool require_coroutine_context(const char* func_name) {
  ...
  return strict_contract_failure(func_name, "must be called in coroutine context", EPERM);
}
...
bool require_nonblocking_fd(const char* func_name, int fd) {
  ...
  return strict_contract_failure(func_name, "fd must be non-blocking", EINVAL);
}
...
void refresh_timeout_cache_after_setsockopt(int fd,
                                            int level,
                                            int option,
  ...
  const bool has_timeval = option_value != nullptr &&
                           option_len >= static_cast<socklen_t>(sizeof(timeval));
  if (option == SO_RCVTIMEO) {
    metadata.recv_timeout_ms = has_timeval
                                   ? timeval_to_milliseconds(*static_cast<const timeval*>(option_value))
                                   : load_socket_timeout_ms(fd, SO_RCVTIMEO);
  } else {
    metadata.send_timeout_ms = has_timeval
                                   ? timeval_to_milliseconds(*static_cast<const timeval*>(option_value))
...
uint32_t remaining_timeout_ms(uint32_t total_timeout_ms,
...
template <typename Func>
ssize_t run_io_loop(const char* func_name,
                    int fd,
                    IoEventType event_type,
                    uint32_t timeout_ms,
                    bool is_read,
                    Func&& func) {
  if (!require_coroutine_context(func_name) || !require_nonblocking_fd(func_name, fd)) {
    return -1;
  }

  const uint32_t effective_timeout_ms = resolve_timeout_ms(fd, timeout_ms, is_read);
  ...
  while (true) {
    ...
    const uint32_t wait_timeout_ms = remaining_timeout_ms(effective_timeout_ms, started_at);
...
int co_socket(int domain, int type, int protocol) {
  ...
  const int fd = static_cast<int>(
      retry_on_eintr([&]() -> ssize_t { return ::socket(domain, type, protocol); }));
#endif
...
int co_tcp_socket(int domain) { return co_socket(domain, SOCK_STREAM, IPPROTO_TCP); }

int co_udp_socket(int domain) { return co_socket(domain, SOCK_DGRAM, IPPROTO_UDP); }
...
int co_dup2(int oldfd, int newfd) {
  const int rc =
      static_cast<int>(retry_on_eintr([&]() -> ssize_t { return ::dup2(oldfd, newfd); }));
  if (rc >= 0) {
...
int co_dup3(int oldfd, int newfd, int flags) {
  ...
  const int rc =
      static_cast<int>(retry_on_eintr([&]() -> ssize_t { return ::dup3(oldfd, newfd, flags); }));
  if (rc >= 0) {
    sync_fd_metadata_on_dup(oldfd, rc);
  }
  return rc;
...
int co_shutdown(int fd, char how) {
  ...
  return static_cast<int>(
      retry_on_eintr([&]() -> ssize_t { return ::shutdown(fd, shutdown_how); }));
}
...
int co_setsockopt(int fd,
  ...
  if (rc == 0) {
    refresh_timeout_cache_after_setsockopt(fd, level, option, option_value, option_len);
  }
  return rc;
}
...
void co_set_nonblock(int fd) {
  if (!force_nonblocking_fd(fd)) {
    ZCOROUTINE_LOG_WARN("co_set_nonblock failed, fd={}, errno={}", fd, errno);
  }
}
...
void co_set_reuseaddr(int fd) {
  const int enabled = 1;
  (void)co_setsockopt(fd, SOL_SOCKET, SO_REUSEADDR, &enabled, sizeof(enabled));
}
...
void co_set_tcp_keepalive(int fd) {
  const int enabled = 1;
  (void)co_setsockopt(fd, SOL_SOCKET, SO_KEEPALIVE, &enabled, sizeof(enabled));
}

void co_set_tcp_nodelay(int fd) {
  const int enabled = 1;
  (void)co_setsockopt(fd, IPPROTO_TCP, TCP_NODELAY, &enabled, sizeof(enabled));
}
...
ssize_t co_recv(int fd, void* buffer, size_t count, int flags, uint32_t timeout_ms) {
  return run_io_loop("co_recv", fd, IoEventType::kRead, timeout_ms, true,
                     [&]() -> ssize_t { return ::recv(fd, buffer, count, flags); });
}

ssize_t co_recvn(int fd, void* buffer, size_t count, int flags, uint32_t timeout_ms) {
  if (!require_coroutine_context("co_recvn") || !require_nonblocking_fd("co_recvn", fd)) {
    return -1;
  }
  ...
  while (received < count) {
    const ssize_t rc = retry_on_eintr(
        [&]() -> ssize_t { return ::recv(fd, current, count - received, flags); });
    if (rc > 0) {
...
ssize_t co_send(int fd, const void* buffer, size_t count, int flags, uint32_t timeout_ms) {
  if (!require_coroutine_context("co_send") || !require_nonblocking_fd("co_send", fd)) {
    return -1;
  }
  ...
  while (sent < count) {
    const ssize_t rc = retry_on_eintr(
        [&]() -> ssize_t { return ::send(fd, current, count - sent, flags); });
    if (rc > 0) {
...
ssize_t co_recvfrom(int fd,
                    ...
                    uint32_t timeout_ms) {
  return run_io_loop("co_recvfrom", fd, IoEventType::kRead, timeout_ms, true, [&]() -> ssize_t {
...
ssize_t co_sendto(int fd,
  ...
  if (!require_coroutine_context("co_sendto") || !require_nonblocking_fd("co_sendto", fd)) {
    return -1;
  }
  ...
  while (sent < count) {
    const ssize_t rc = retry_on_eintr([&]() -> ssize_t {
      return ::sendto(fd, current, count - sent, flags, address, address_len);
    });
    if (rc > 0) {
      sent += static_cast<size_t>(rc);
      current += rc;
...
int co_connect(int fd, const struct sockaddr* address, socklen_t address_len, uint32_t timeout_ms) {
  if (!require_coroutine_context("co_connect") || !require_nonblocking_fd("co_connect", fd)) {
    return -1;
  }
  ...
  while (true) {
    const int rc =
        static_cast<int>(retry_on_eintr([&]() -> ssize_t { return ::connect(fd, address, address_len); }));
    if (rc == 0 || errno == EISCONN) {
      return 0;
    }

    if (errno != EINPROGRESS && errno != EALREADY && !is_retryable_errno(errno)) {
      return -1;
    }
    ...
    if (co_getsockopt(fd, SOL_SOCKET, SO_ERROR, &socket_error, &socket_error_len) != 0) {
      return -1;
    }
...
int co_accept(int fd, struct sockaddr* address, socklen_t* address_len, uint32_t timeout_ms) {
  if (!require_coroutine_context("co_accept") || !require_nonblocking_fd("co_accept", fd)) {
    return -1;
  }
  ...
  while (true) {
    const int accepted_fd =
        static_cast<int>(retry_on_eintr([&]() -> ssize_t { return ::accept(fd, address, address_len); }));
    if (accepted_fd >= 0) {
...
int co_accept4(int fd,
               ...
               uint32_t timeout_ms) {
#if defined(__linux__)
  if (!require_coroutine_context("co_accept4") || !require_nonblocking_fd("co_accept4", fd)) {
    return -1;
  }
  ...
  while (true) {
    const int accepted_fd = static_cast<int>(
        retry_on_eintr([&]() -> ssize_t { return ::accept4(fd, address, address_len, syscall_flags); }));
    if (accepted_fd >= 0) {
      sync_fd_metadata_on_dup(fd, accepted_fd);
      return accepted_fd;
    }
