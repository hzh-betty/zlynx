...
struct Event::Impl {
  ...
  Impl(bool manual_reset_arg, bool signaled_arg)
      : manual_reset(manual_reset_arg), signaled(signaled_arg), mutex(), cv(), waiters() {}

  /**
   * @brief 清理无效等待节点。
   * @return 无返回值。
   */
  void cleanup_waiters() {
    zcoroutine::cleanup_waiters(&waiters);
  }
...
bool Event::wait(uint32_t milliseconds) const {
  ...
  if (!in_coroutine()) {
    ...
    const bool ok = impl_->cv.wait_for(lock, std::chrono::milliseconds(milliseconds),
                                       [this]() { return impl_->signaled; });
    if (!ok) {
      ZCOROUTINE_LOG_DEBUG("event wait timeout in thread context, timeout_ms={}", milliseconds);
      return false;
    }
  ...
  const bool ok = milliseconds == kInfiniteTimeoutMs ? park_current()
                                                      : park_current_for(milliseconds);
  if (!ok) {
    active->store(false, std::memory_order_release);
    ZCOROUTINE_LOG_DEBUG("event wait timeout in coroutine context, timeout_ms={}", milliseconds);
  }
  return ok;
}
...
void Event::signal() const {
  ...
  std::vector<Fiber::ptr> resume_list;
  {
    ...
    if (impl_->manual_reset) {
      ...
      // auto_reset：只唤醒一个等待协程；若当前无等待者，则缓存 signaled 状态。
      // 缓存的 signaled 会在下一次 wait 进入时被消费。
      bool resumed = false;
      for (size_t i = 0; i < impl_->waiters.size(); ++i) {
        CoroutineWaiterEntry& waiter = impl_->waiters[i];
        Fiber::ptr coroutine = claim_waiter(&waiter);
  ...
  ZCOROUTINE_LOG_DEBUG("event signal wake coroutine count={}", resume_list.size());
...
void Event::notify_all() const {
  ...
  std::vector<Fiber::ptr> resume_list;
  {
    ...
    // auto_reset 下 notify_all 后不保留 signaled；manual_reset 下保持触发态。
    impl_->cleanup_waiters();
  }

  impl_->cv.notify_all();

  ZCOROUTINE_LOG_DEBUG("event notify_all wake coroutine count={}", resume_list.size());
