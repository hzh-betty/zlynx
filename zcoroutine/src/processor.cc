...
void Processor::start() {
    ...
    ZCOROUTINE_LOG_INFO("processor started, sched_id={}, stack_size={}", id_,
                        stack_size_);
}
...
void Processor::enqueue_task(Task task) {
    ...
    ZCOROUTINE_LOG_DEBUG("task enqueued, sched_id={}, pending_tasks={}", id_,
                         steal_queue_.size());
    wake_loop();
}
...
void Processor::enqueue_ready(Fiber::ptr fiber) {
...

    {
        ...
        ZCOROUTINE_LOG_DEBUG("fiber ready enqueued, sched_id={}, ready_size={}",
                             id_, run_queue_.size());
    }
...
Processor::add_timer(uint32_t milliseconds, std::function<void()> callback) {
    ...
    ZCOROUTINE_LOG_DEBUG("timer added, sched_id={}, delay_ms={}", id_,
                         milliseconds);
    wake_loop();
    return token;
}
...
bool Processor::wait_fd(int fd, uint32_t events, uint32_t milliseconds) {
    ...
    ZCOROUTINE_LOG_DEBUG("wait_fd start, sched_id={}, fiber_id={}, fd={}, "
                         "events={}, timeout_ms={}",
...
bool Processor::recycle_if_done_before_run(const Fiber::ptr &fiber) {
    ...
    ZCOROUTINE_LOG_DEBUG("skip done fiber before run, sched_id={}, fiber_id={}",
                         id_, fiber->id());
...
Fiber::ptr Processor::switch_to_fiber(Fiber::ptr fiber) {
    ...
    ZCOROUTINE_LOG_DEBUG("switch to fiber, sched_id={}, fiber_id={}", id_,
                         current_fiber_->id());
...
void Processor::handle_io_ready(const std::shared_ptr<IoWaiter> &waiter,
    ...
    if (Fiber::ptr fiber = waiter->fiber.lock()) {
        ZCOROUTINE_LOG_DEBUG("io ready resume fiber, sched_id={}, fd={}, "
...
void Processor::save_fiber_stack(const Fiber::ptr &fiber) {
    ...
    if (stack_sp < stack_bottom || stack_sp > stack_top) {
        ZCOROUTINE_LOG_WARN("shared stack save failed, sp out of range, "
...
void Processor::restore_fiber_stack(const Fiber::ptr &fiber) {
    ...
    if (used > stack_size) {
        ZCOROUTINE_LOG_WARN("shared stack restore failed, snapshot too large, "
