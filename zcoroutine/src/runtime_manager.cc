...
void Runtime::init(uint32_t scheduler_count) {
    ...
    ZCOROUTINE_LOG_INFO("runtime initialized, scheduler_count={}",
                        processors_.size());
}
...
void Runtime::register_fiber(const Fiber::ptr &fiber) {
    ...
    ZCOROUTINE_LOG_DEBUG("fiber registered, fiber_id={}, handle_id={}",
...
void Runtime::unregister_fiber(Fiber *fiber) {
    ...
    ZCOROUTINE_LOG_DEBUG("fiber unregistered, fiber_id={}, handle_id={}",
                         fiber->id(), handle_id);
}
