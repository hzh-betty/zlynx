...
char* SnapshotBufferPool::acquire(size_t required_size, size_t* capacity, uint8_t* bucket_index) {
  if (required_size == 0) {
    if (capacity) {
      *capacity = 0;
    }
...
void SnapshotBufferPool::release(char* buffer, uint8_t bucket_index, size_t capacity) {
  if (!buffer) {
    return;
  }
