...
Epoller::Epoller()
    : epoll_fd_(-1), wake_fd_(-1), wake_pending_(false), waiter_mutex_(), fd_wait_states_() {}
...
bool Epoller::start() {
  ...
  if (epoll_fd_ < 0 || wake_fd_ < 0) {
    ZCOROUTINE_LOG_FATAL("epoller init failed, epoll_fd={}, wake_fd={}, errno={}", epoll_fd_,
                         wake_fd_, errno);
    stop();
    return false;
  }
  ...
  if (epoll_ctl(epoll_fd_, EPOLL_CTL_ADD, wake_fd_, &wake_ev) != 0) {
    ZCOROUTINE_LOG_FATAL("epoller register wake fd failed, wake_fd={}, errno={}", wake_fd_, errno);
    stop();
    return false;
  }

  ZCOROUTINE_LOG_INFO("epoller started, epoll_fd={}, wake_fd={}", epoll_fd_, wake_fd_);
  return true;
}
...
void Epoller::wake() {
  ...
  if (rc < 0 && errno != EAGAIN) {
    ...
    ZCOROUTINE_LOG_WARN("epoller wake failed, wake_fd={}, errno={}", wake_fd_, errno);
  }
}
...
void Epoller::unregister_waiter(const std::shared_ptr<IoWaiter>& waiter) {
  ...
  if ((waiter->events & EPOLLOUT) && state.write_waiter.get() == waiter.get()) {
...
bool Epoller::update_interest_locked(int fd, FdWaitState* state) {
  ...
  if (state->read_waiter && state->read_waiter->active.load(std::memory_order_acquire)) {
    desired_events |= EPOLLIN;
  }
  if (state->write_waiter && state->write_waiter->active.load(std::memory_order_acquire)) {
  ...
  if (desired_events == 0) {
    if (state->registered) {
      if (epoll_ctl(epoll_fd_, EPOLL_CTL_DEL, fd, nullptr) != 0 && errno != ENOENT) {
        ZCOROUTINE_LOG_WARN("epoll del waiter failed, fd={}, errno={}", fd, errno);
        return false;
      }
    }
  ...
  if (rc != 0) {
    ZCOROUTINE_LOG_WARN("epoll add/mod waiter failed, fd={}, events={}, errno={}", fd,
...
void Epoller::wait_events(
  ...
  const int event_count = epoll_wait(epoll_fd_, events, kMaxEpollEvents, timeout_ms);
  if (event_count <= 0) {
  ...
  for (int i = 0; i < event_count; ++i) {
    ...
    FdWaitState& state = it->second;
    const bool read_ready = (ready_events & (EPOLLIN | EPOLLHUP | EPOLLERR)) != 0;
    const bool write_ready = (ready_events & (EPOLLOUT | EPOLLHUP | EPOLLERR)) != 0;

    if (read_ready && state.read_waiter) {
      ready_waiters.push_back(std::make_pair(state.read_waiter, ready_events));
      state.read_waiter.reset();
    }

    if (write_ready && state.write_waiter) {
      ready_waiters.push_back(std::make_pair(state.write_waiter, ready_events));
      state.write_waiter.reset();
    }
  ...
  if (event_count > 0) {
    ZCOROUTINE_LOG_DEBUG("epoll wait returned events, count={}, timeout_ms={}", event_count,
                         timeout_ms);
  }
}
