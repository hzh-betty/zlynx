...
struct Mutex::Impl {
  void cleanup_waiters_locked() {
    cleanup_waiters_front(&coroutine_waiters);
  }
...
void Mutex::lock() const {
  if (!impl_) {
    ZCOROUTINE_LOG_ERROR("mutex lock failed, impl is null");
    return;
  }
  ...
  std::shared_ptr<std::atomic<bool>> active = std::make_shared<std::atomic<bool>>(true);
  {
    ...
    impl_->coroutine_waiters.push_back(CoroutineWaiterEntry{coroutine, active});
...
void Mutex::unlock() const {
  if (!impl_) {
    ZCOROUTINE_LOG_WARN("mutex unlock ignored, impl is null");
    return;
  }
  ...
  Fiber::ptr resume_target;
  {
    std::lock_guard<std::mutex> lock(impl_->mutex);
    if (!impl_->locked) {
      ZCOROUTINE_LOG_WARN("mutex unlock ignored, mutex is not locked");
      return;
    }
...
bool Mutex::try_lock() const {
  if (!impl_) {
    ZCOROUTINE_LOG_WARN("mutex try_lock failed, impl is null");
    return false;
  }
