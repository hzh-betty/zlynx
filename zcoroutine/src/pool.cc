...
struct Pool::Impl {
    ...
    void *pop() {
        ...
        if (!bucket.empty()) {
            ...
            bucket.pop_back();
            ZCOROUTINE_LOG_DEBUG("pool pop from bucket, remaining_size={}",
                                 bucket.size());
            return element;
        }
    ...
    void push(void *element) {
        ...
        if (destroy_cb && capacity != static_cast<size_t>(-1) &&
            ...
            // 容量受限时即时回收，避免桶无限增长。
            ZCOROUTINE_LOG_DEBUG("pool push triggers destroy callback, "
        ...
        ZCOROUTINE_LOG_DEBUG("pool push success, bucket_size={}",
                             bucket.size());
    }
    ...
    void clear_all() {
        ...
        buckets.clear();
        ZCOROUTINE_LOG_INFO("pool clear all finished, released_count={}",
                            release_count);
    }
