...
struct Pool::Impl {
   ...
   */
  Impl(CreateCallback create_cb_arg, DestroyCallback destroy_cb_arg, size_t capacity_arg)
      : create_cb(std::move(create_cb_arg)),
        destroy_cb(std::move(destroy_cb_arg)),
  ...
  void* pop() {
    ...
    if (!bucket.empty()) {
      ...
      bucket.pop_back();
      ZCOROUTINE_LOG_DEBUG("pool pop from bucket, remaining_size={}", bucket.size());
      return element;
    }

    if (create_cb) {
      ZCOROUTINE_LOG_DEBUG("pool pop triggers create callback");
      return create_cb();
    }
    ZCOROUTINE_LOG_DEBUG("pool pop returns null, no cached element and no create callback");
    return nullptr;
  }
  ...
  void push(void* element) {
    ...
    std::lock_guard<std::mutex> lock(mutex);
    std::vector<void*>& bucket = buckets[std::this_thread::get_id()];
    if (destroy_cb && capacity != static_cast<size_t>(-1) && bucket.size() >= capacity) {
      // 容量受限时即时回收，避免桶无限增长。
      ZCOROUTINE_LOG_DEBUG("pool push triggers destroy callback, bucket_size={}, capacity={}",
                           bucket.size(), capacity);
      destroy_cb(element);
      return;
    }
    bucket.push_back(element);
    ZCOROUTINE_LOG_DEBUG("pool push success, bucket_size={}", bucket.size());
  }
  ...
  void clear_all() {
    ...
    buckets.clear();
    ZCOROUTINE_LOG_INFO("pool clear all finished, released_count={}", release_count);
  }
...
Pool::Pool(CreateCallback create_cb, DestroyCallback destroy_cb, size_t capacity)
    : impl_(std::make_shared<Impl>(std::move(create_cb), std::move(destroy_cb), capacity)) {
  ZCOROUTINE_LOG_INFO("pool created, capacity={}", capacity);
}
