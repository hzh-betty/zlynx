...
void WaitGroup::add(uint32_t count) {
  ...
  ZCOROUTINE_LOG_DEBUG("wait_group add, delta={}, previous={}, current={}", delta, previous,
                       previous + delta);
  if (previous == 0 && delta > 0) {
    // 从 0 变为正值时，说明需要重新进入等待态。
    // 注意：调用方应避免与 wait() 产生业务层竞态（与 Go WaitGroup 使用约束一致）。
    done_event_.reset();
  }
}
...
void WaitGroup::done() {
  ...
  if (previous <= 0) {
    ...
    ZCOROUTINE_LOG_ERROR("wait_group done failed, counter would become negative");
