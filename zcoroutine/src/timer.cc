...
std::shared_ptr<TimerToken> TimerQueue::add_timer(uint32_t milliseconds, std::function<void()> callback) {
...

  {
    ...
    ZCOROUTINE_LOG_DEBUG("timer queued, delay_ms={}, queue_size={}", milliseconds, timers_.size());
  }
...
void TimerQueue::process_due() {
  ...
  while (true) {
    ...
    if (entry.token && entry.token->cancelled.load(std::memory_order_acquire)) {
      continue;
    }

    if (entry.callback) {
      ZCOROUTINE_LOG_DEBUG("timer fired, deadline_ms={}", entry.deadline_ms);
      entry.callback();
    }
  }
}
