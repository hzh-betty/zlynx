...
void test_zlog_sync(int thread_count, size_t message_size) {
    ...
    sinks.push_back(
        std::make_shared<zlog::FileSink>("bench_logs/zlog_sync.log"));
...
void test_spdlog_async(int thread_count, size_t message_size) {
    ...
    auto result = run_timed_benchmark("Spdlog-Async", thread_count,
...
void print_results() {
    ...
    std::cout << std::left << std::setw(22) << "Logger Type" << std::setw(10)
              ...
              << "Msg Size(B)" << std::setw(15) << "Duration(s)"
    ...
    for (const auto &r : g_results) {
        std::cout << std::left << std::setw(22) << r.logger_type
                  << std::setw(10) << r.thread_count << std::setw(18)
                  << r.total_messages << std::setw(15) << r.message_size
                  << std::setw(15) << std::fixed << std::setprecision(2)
                  << r.duration << std::setw(18) << std::fixed
                  << std::setprecision(2) << r.throughput_mbps << "\n";
    }
...
int main() {
    ...
    for (int threads : thread_counts) {
        for (size_t msg_size : message_sizes) {
            std::cout << "[" << threads << " threads, " << msg_size
                      << " bytes] ";
