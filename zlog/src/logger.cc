...
SyncLogger::SyncLogger(const char *logger_name, const LogLevel::value limit_level,
...
AsyncLogger::AsyncLogger(const char *logger_name,
    ...
    : Logger(logger_name, limit_level, formatter, sinks),
      looper_(std::make_shared<AsyncLooper>(
          AsyncLooper::Functor{[this](const Buffer &buf) { this->re_log(buf); }},
...
LoggerBuilder::LoggerBuilder()
    : logger_type_(LoggerType::LOGGER_SYNC), limit_level_(LogLevel::value::DEBUG),
...
Logger::ptr LocalLoggerBuilder::build() {
  ...
  if (logger_type_ == LoggerType::LOGGER_ASYNC) {
    return std::make_shared<AsyncLogger>(logger_name_, limit_level_, formatter_,
                                         sinks_, looper_type_, milliseco_);
  }
...
Logger::ptr GlobalLoggerBuilder::build() {
  ...
  Logger::ptr logger;
  if (logger_type_ == LoggerType::LOGGER_ASYNC) {
    logger = std::make_shared<AsyncLogger>(logger_name_, limit_level_, formatter_,
                                           sinks_, looper_type_, milliseco_);
  } else {
    logger = std::make_shared<SyncLogger>(logger_name_, limit_level_, formatter_,
                                          sinks_);
  }
