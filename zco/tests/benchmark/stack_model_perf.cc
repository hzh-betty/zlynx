...
WorkloadConfig load_workload_config() {
    ...
    config.scheduler_tasks =
        scaled_workload(read_env_int("ZCO_PERF_SCHED_TASKS",
...
void print_result(const ScenarioResult &result) {
    std::cout << "[zco-perf] scenario=" << result.scenario
...
int run_stack_model_perf_main() {
    try {
        ...
        shutdown();
        std::cerr << "[zco-perf] error=" << ex.what() << std::endl;
        return 1;
    } catch (...) {
        shutdown();
        std::cerr << "[zco-perf] error=unknown_exception" << std::endl;
        return 1;
    }
}
