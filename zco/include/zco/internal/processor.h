...
class Processor : public NonCopyable {
  public:
    ...
    Processor(int id, size_t stack_size, size_t shared_stack_num,
     ...
     * @return 实际窃取数量。
     */
    size_t steal_tasks(std::deque<Task> *tasks, size_t max_steal,
                       size_t min_reserve);
     ...
     * @return 缓冲地址。
     */
    char *acquire_snapshot_buffer(size_t required_size, size_t *capacity,
     ...
     * @return 无返回值。
     */
    void release_snapshot_buffer(char *buffer, uint8_t bucket_index,
                                 size_t capacity);
...
#endif // ZCO_INTERNAL_PROCESSOR_H_
