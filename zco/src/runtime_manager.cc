...
Runtime::Runtime()
    : started_(false), rr_index_(0),
      chooser_seed_(static_cast<uint64_t>(
          std::chrono::steady_clock::now().time_since_epoch().count())),
      fiber_id_gen_(1), fiber_handle_id_gen_(1), processors_(),
      stack_config_mutex_(), stack_num_(kDefaultSharedStackNum),
      stack_size_(kDefaultStackSize), stack_model_(kDefaultStackModel),
      scheduler_handle_mutex_(), scheduler_handles_(),
      fiber_handle_registry_() {}

bool Runtime::set_stack_num(size_t stack_num) {
    if (stack_num == 0) {
        ZCO_LOG_WARN("co_stack_num ignored invalid value 0");
        return false;
    }

    std::lock_guard<std::mutex> lock(stack_config_mutex_);
    if (started_.load(std::memory_order_acquire)) {
        ZCO_LOG_WARN("co_stack_num must be called before runtime start");
        return false;
    }
...
bool Runtime::set_stack_size(size_t stack_size) {
    if (stack_size == 0) {
        ZCO_LOG_WARN("co_stack_size ignored invalid value 0");
        return false;
    }
...
Scheduler *Runtime::ensure_scheduler_handle(size_t scheduler_index) {
    ...
    while (scheduler_handles_.size() <= scheduler_index) {
        scheduler_handles_.push_back(std::unique_ptr<Scheduler>(
            new Scheduler(scheduler_handles_.size())));
    }
...
void Runtime::init(uint32_t scheduler_count) {
    ...
    if (!started_.compare_exchange_strong(expected, true,
        ...
        ZCO_LOG_DEBUG("runtime init skipped, already started");
        return;
    }
    ...
    for (uint32_t i = 0; i < final_count; ++i) {
        ...
        processors_.push_back(std::unique_ptr<Processor>(new Processor(
            static_cast<int>(i), stack_size, stack_num, stack_model)));
    }
...
void Runtime::shutdown() {
    ...
    if (!started_.compare_exchange_strong(expected, false,
                                          std::memory_order_acq_rel)) {
        ZCO_LOG_DEBUG("runtime shutdown skipped, not started");
        return;
    }
    ...
    ZCO_LOG_INFO("runtime shutdown completed");
}

void Runtime::submit(Task task) {
    if (!task) {
        ZCO_LOG_WARN("submit ignored null task");
        return;
    }
...
size_t Runtime::pick_secondary_index(size_t first, uint64_t ticket) {
    ...
    uint64_t x = ticket ^ chooser_seed_.fetch_add(0x9e3779b97f4a7c15ULL,
...
void Runtime::resume_external(void *handle) {
    if (!handle) {
        ZCO_LOG_WARN("resume_external ignored null fiber");
        return;
    }
    ...
    if (!holder) {
        ZCO_LOG_WARN(
            "resume_external failed, fiber handle not found, handle_id={}",
            handle_id);
        return;
    }
...
const std::vector<std::unique_ptr<Processor>> &Runtime::processors() const {
    return processors_;
}

int Runtime::next_fiber_id() {
    return fiber_id_gen_.fetch_add(1, std::memory_order_relaxed);
}
...
void resume_fiber(const Fiber::ptr &fiber, bool timed_out) {
    ...
    if (!fiber->try_wake(timed_out)) {
        ZCO_LOG_DEBUG(
            "resume_fiber ignored, fiber not in waiting state, fiber_id={}",
            fiber->id());
        return;
    }
