...
Fiber::Fiber(int id, Processor *owner, Task task, size_t stack_size,
    ...
    if (use_shared_stack_) {
        if (owner_->shared_stack_count() == 0 ||
            stack_slot_ >= owner_->shared_stack_count() ||
            owner_->shared_stack_size(stack_slot_) == 0 ||
            owner_->shared_stack_data(stack_slot_) == nullptr) {
    ...
    ZCO_LOG_DEBUG(
        "fiber created(independent stack lazy init), fiber_id={}, sched_id={}, "
        "stack_size={}",
...
bool Fiber::try_set_external_handle_id(uint64_t handle_id,
                                       uint64_t *effective_handle) {
    if (handle_id == 0 || !effective_handle) {
        return false;
    }

    uint64_t expected = 0;
    if (external_handle_id_.compare_exchange_strong(
            expected, handle_id, std::memory_order_acq_rel,
...
Fiber::State Fiber::state() const {
    return state_.load(std::memory_order_acquire);
}

void Fiber::mark_running() {
    state_.store(State::kRunning, std::memory_order_release);
}

void Fiber::mark_ready() {
    state_.store(State::kReady, std::memory_order_release);
}
...
void Fiber::run() {
    ...
    mark_done();
    ZCO_LOG_DEBUG("fiber finished, fiber_id={}", id_);
}
...
void Fiber::save_stack_data(const char *data, size_t size) {
    ...
    if (!saved_stack_buffer_ || saved_stack_capacity_ < size) {
        clear_saved_stack();
        saved_stack_buffer_ = owner_->acquire_snapshot_buffer(
            size, &saved_stack_capacity_, &saved_stack_bucket_);
        if (!saved_stack_buffer_ || saved_stack_capacity_ < size) {
...
bool Fiber::has_saved_stack() const {
    return saved_stack_buffer_ != nullptr && saved_stack_size_ != 0;
}
...
void Fiber::clear_saved_stack() {
    if (saved_stack_buffer_ && owner_) {
        owner_->release_snapshot_buffer(
            saved_stack_buffer_, saved_stack_bucket_, saved_stack_capacity_);
    }
