...
WaitGroup::WaitGroup(uint32_t count)
    ...
    ZCO_LOG_DEBUG("wait_group created, initial_count={}", count);
}
...
void WaitGroup::done() {
    ...
    if (previous == 1) {
        ...
        ZCO_LOG_DEBUG("wait_group reached zero, signal waiters");
        done_event_.signal();
    }
}
...
void WaitGroup::wait() {
    ...
    ZCO_LOG_DEBUG("wait_group wait begin");
    (void)done_event_.wait();
    ZCO_LOG_DEBUG("wait_group wait end");
}
