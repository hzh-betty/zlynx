...
struct Event::Impl {
     ...
     * @return 无返回值。
     */
    void cleanup_waiters() { zco::cleanup_waiters(&waiters); }
...
bool Event::wait(uint32_t milliseconds) const {
    if (!impl_) {
        ZCO_LOG_ERROR("event wait failed, impl is null");
        return false;
    }
...
void Event::signal() const {
    if (!impl_) {
        ZCO_LOG_WARN("event signal ignored, impl is null");
        return;
    }
...
void Event::reset() const {
    if (!impl_) {
        ZCO_LOG_WARN("event reset ignored, impl is null");
        return;
    }

    std::lock_guard<std::mutex> lock(impl_->mutex);
    impl_->signaled = false;
    ZCO_LOG_DEBUG("event reset done");
}

void Event::notify_all() const {
    if (!impl_) {
        ZCO_LOG_WARN("event notify_all ignored, impl is null");
        return;
    }
