 ...
 * - 通过 Socket 封装完成收发，复用协程 Hook 语义并减少裸系统调用。
 */
class Buffer : public NonCopyable {
  public:
     ...
     */
    ssize_t read_from_socket(const std::shared_ptr<Socket> &socket,
                             size_t max_read_bytes, uint32_t timeout_ms,
                             int *saved_errno);

    /**
     * @brief 将当前可读数据写入 Socket。
     * @param socket 目标 socket。
     * @param timeout_ms 本次写入超时（毫秒），0 表示无限等待。
     * @param saved_errno 失败时写回 errno，可为空。
     * @return >0 写出字节数，0 表示无数据可写，<0 写入失败。
     */
    ssize_t write_to_socket(const std::shared_ptr<Socket> &socket,
