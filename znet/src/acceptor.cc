...
bool Acceptor::start() {
  ...
  if (!running_.compare_exchange_strong(expected, true)) {
    ZNET_LOG_DEBUG("Acceptor::start skipped because acceptor is already running");
    return true;
  }
  ...
  if (!listen_socket_->bind(listen_address_) || !listen_socket_->listen(backlog_)) {
...
void Acceptor::stop() {
  ...
  if (!running_.compare_exchange_strong(expected, false)) {
    ZNET_LOG_DEBUG("Acceptor::stop skipped because acceptor is not running");
    return;
  }
...
void Acceptor::accept_loop() {
  ...
  while (running_.load()) {
    if (!listen_socket_) {
      ZNET_LOG_WARN("Acceptor::accept_loop exits because listen socket is null");
      break;
    }
    ...
    if (!client) {
      if (!running_.load()) {
        ZNET_LOG_DEBUG("Acceptor::accept_loop exits because acceptor stopped");
        break;
      }
      ...
      if (errno == EBADF) {
        ZNET_LOG_DEBUG("Acceptor::accept_loop exits because listen fd is closed");
        break;
      }
    ...
    ZNET_LOG_DEBUG("Acceptor::accept_loop accepted client: fd={}", client->fd());

    if (accept_callback_) {
      // 连接交由上层生命周期管理，Acceptor 仅负责分发。
      accept_callback_(std::move(client));
    } else {
      ZNET_LOG_WARN("Acceptor::accept_loop dropped client because callback is null");
    }
  }
