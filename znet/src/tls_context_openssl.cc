 ...
 public:
  ...
  bool handshake(uint32_t timeout_ms,
    ...
    while (true) {
      ...
      if (ssl_error == SSL_ERROR_WANT_READ || ssl_error == SSL_ERROR_WANT_WRITE) {
        if (!wait_callback ||
            !wait_callback(ssl_error == SSL_ERROR_WANT_WRITE, timeout_ms)) {
          if (errno == 0) {
            errno = ETIMEDOUT;
          }
          return false;
  ...
  ssize_t write(const void* buffer,
    ...
    while (sent < length) {
      ...
      ERR_clear_error();
      const int n =
          SSL_write(ssl_, static_cast<const char*>(buffer) + sent, chunk);
      if (n > 0) {
...
TlsContext::ptr create_server_tls_context_openssl(const std::string& cert_file,
  ...
  if (SSL_CTX_use_certificate_file(raw_ctx, cert_file.c_str(), SSL_FILETYPE_PEM) <=
      0) {
    if (error) {
  ...
  if (SSL_CTX_use_PrivateKey_file(raw_ctx, key_file.c_str(), SSL_FILETYPE_PEM) <=
      0) {
    if (error) {
  ...
  ZNET_LOG_INFO(
      "create_server_tls_context_openssl success: cert={}, key={}", cert_file,
      key_file);
