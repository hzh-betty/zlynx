...
int scaled_value(const int base, const int scale_pct, const int min_value) {
  const long long scaled =
      (static_cast<long long>(base) * static_cast<long long>(scale_pct)) / 100;
  if (scaled < min_value) {
    return min_value;
  }
...
std::string extract_metric(const std::string& output, const std::string& key) {
  ...
  return trim(output.substr(pos + needle.size(), line_end - pos - needle.size()));
}
...
void print_usage(const char* prog) {
  std::cout
      ...
      << "  --warmup-ms N                 Delay before wrk exec (default 300)\n"
      << "  --path STR                    Request path (default /)\n"
      << "  --scale-pct N                 Workload scale percent (default 100)\n"
      << "  --server-ready-timeout-ms N   Server ready timeout (default 3000)\n"
...
BenchConfig parse_args(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (std::strcmp(arg, "--wrk-threads") == 0) {
      int value = 0;
      if (!parse_int(require_next("--wrk-threads"), &value) || value <= 0) {
    ...
    if (std::strcmp(arg, "--wrk-connections") == 0) {
      int value = 0;
      if (!parse_int(require_next("--wrk-connections"), &value) || value <= 0) {
...
void apply_scale(BenchConfig* cfg) {
  ...
  cfg->wrk_connections = scaled_value(cfg->wrk_connections, cfg->scale_pct, 1);
}
...
class HelloWorldHandler {
 public:
  void on_message(const znet::TcpConnection::ptr& conn, znet::Buffer& buffer) {
    if (!conn) {
      return;
    }
    ...
    static const std::string kResponseKeepAlive =
        ...
        "\r\n"
        "hello world";
    static const std::string kResponseClose =
        "HTTP/1.1 200 OK\r\n"
        "Server: znet-wrk-bench\r\n"
        "Content-Type: text/plain\r\n"
        "Content-Length: 11\r\n"
    ...
    while (buffer.readable_bytes() > 0) {
      ...
      const char* header_end = std::search(begin, end, kHeaderEnd, kHeaderEnd + 4);
      if (header_end == end) {
        break;
      }
      ...
      const bool wants_close = request_contains_connection_close(begin, request_size);
      buffer.retrieve(request_size);

      const std::string& response = wants_close ? kResponseClose : kResponseKeepAlive;
...
int run_server_process(const BenchConfig& cfg, const int ready_fd) {
  ...
  auto address =
      std::make_shared<znet::IPv4Address>("127.0.0.1", static_cast<uint16_t>(cfg.port));
  ...
  server->set_on_message([&handler](const znet::TcpConnection::ptr& conn,
                                    znet::Buffer& buffer) {
...
WrkResult run_wrk(const BenchConfig& cfg) {
  ...
  if (!find_executable(cfg.wrk_bin, &resolved)) {
    ...
    result.output = "wrk binary not found or not executable: " + cfg.wrk_bin;
    return result;
  }
...
void print_config(const BenchConfig& cfg) {
  std::cout << "[znet-wrk-bench] config"
            << " port=" << cfg.port
