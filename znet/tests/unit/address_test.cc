...
class AddressUnitTest : public ::testing::Test {};

TEST_F(AddressUnitTest, LookupReturnsResultForLocalhost) {
...
TEST_F(AddressUnitTest, LookupReturnsEmptyOnInvalidHost) {
    const auto result = Address::lookup("invalid host name", 10000);
    EXPECT_TRUE(result.empty());
}

TEST_F(AddressUnitTest, CreateHandlesNullAndUnknownFamily) {
...
TEST_F(AddressUnitTest, CreateDispatchesToConcreteAddressTypes) {
    sockaddr_in v4;
...
TEST_F(AddressUnitTest, IPv4AndIPv6FallbackAndPortMutationWork) {
...
TEST_F(AddressUnitTest, UnixAddressTruncatesLongPathSafely) {
    UnixAddress addr;
