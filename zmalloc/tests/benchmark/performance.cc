...
static void print_usage(const char* argv0) {
  std::cout
      ...
      << "  -s, --size BYTES       Allocation size in bytes (default: 64)\n"
      << "  -n, --allocs N         Allocations per thread per round (default: 100000)\n"
      << "  -r, --rounds N         Rounds (default: 10)\n"
      << "      --touch            Touch allocated memory (write 1 byte)\n"
      << "  -h, --help             Show this help\n\n"
      << "Examples:\n"
      << "  " << argv0
      << " --threads 8 --size 64 --allocs 200000 --rounds 20\n"
      << "  " << argv0
      << " -t 4 -s 1024 -n 50000 -r 30 --touch\n";
}
...
#endif
static void do_not_optimize_ptr(void* p) {
#if defined(__GNUC__) || defined(__clang__)
...
static long long run_zmalloc(const Options& opt) {
  ...
  auto worker = [&](int /*tid*/) {
    ...
    for (size_t r = 0; r < opt.rounds; ++r) {
      ...
      for (size_t i = 0; i < opt.num_allocs; ++i) {
        void* p = zmalloc::zmalloc(opt.alloc_size);
  ...
  do_not_optimize_ptr(reinterpret_cast<void*>(sink.load(std::memory_order_relaxed)));
...
int main(int argc, char** argv) {
  ...
  if (!parse_args(argc, argv, &opt)) {
    // If user asked for help, parse_args already printed usage and returned false.
    // For parse errors, it printed error + usage.
    return 1;
  }
  ...
  std::cout << "==============================" << std::endl;
  std::cout << "threads=" << opt.threads << ", size=" << opt.alloc_size
            << ", allocs=" << opt.num_allocs << ", rounds=" << opt.rounds
            << ", touch=" << (opt.touch_memory ? "true" : "false")
            << std::endl;
