...
void ThreadCache::deallocate(void *ptr, size_t size) {
    ...
    if (!kEnableThreadCacheLocalFreeList) {
        ...
        CentralCache::get_instance().release_list_to_spans(ptr, align_size,
                                                           index);
        return;
    }
    ...
    if (ZM_UNLIKELY(free_lists_[index].size() >=
                    free_lists_[index].max_size())) {
